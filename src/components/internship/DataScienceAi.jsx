import React from "react";
const sections = {
  DataScienceWithAi: [
    {
      title: "Data Science with AI",
      description:
        "Crafted by industry experts, this rigorous program establishes a strong foundation in data science, machine learning, and AI-driven analytics. Whether you're a beginner or an aspiring professional, this training merges cutting-edge tools with real-world applications, empowering you to extract actionable insights and build intelligent systems with precision and confidence.",
      image: "/DataScience.png",
      modules: [
        "Phase 1: IDE & Server Management",
        "Phase 2: Core Python",
        "Phase 3: Advanced Python",
        "Phase 4: Python Database Connection",
        "Phase 5: Introduction to Business Analytics",
        "Phase 6: Introduction to Data Science",
        "Phase 7: Machine Learning",
        "Phase 8: Exploring Python",
        "Phase 9: Model Creation",
        "Phase 10: Model Evaluation",
        "Phase 11:Deep Learning",
        "Phase 12: Feature Engineering",
        "Phase 13: Project",
        "Phase 14: Allocation to Client & Customers Live Project",
      ],
      duration: "8–12 Weeks",
    },
  ],
};
const DataScienceAi = () => {
  const course = sections.DataScienceWithAi[0];
  return (
    <div className="mt-30 px-6 flex flex-col  gap-8 p-4">
      <div className="flex flex-col md:flex-row gap-6 items-start">
        <div className="md:w-2/3">
          <h2 className="text-2xl md:text-3xl font-medium text-orange-500 mb-4">
            {course.title}
          </h2>
          <p className="text-gray-700 text-sm md:text-base mt-4 leading-relaxed">
            {course.description}
          </p>
        </div>
        <div className="md:w-1/3">
          <img
            src={course.image}
            alt={course.title}
            className="rounded-lg shadow-md w-full hover:scale-[1.02] transition transform duration-300"
          />
        </div>
      </div>
      <div className="flex flex-col md:flex-row items-start relative">
        <div className="w-full md:w-2/5 border p-4 relative z-10">
          <h3 className="text-lg font-semibold text-orange-500 mb-2">
            Training Modules
          </h3>
          <ul className="list-disc pl-5 space-y-1 text-sm text-gray-600">
            {course.modules.map((module, index) => (
              <li key={index}>{module}</li>
            ))}
          </ul>
        </div>

        <div className="hidden md:block w-50 h-1 bg-orange-500 self-center -ml-px mx-2 mt-4"></div>

        <div className="flex items-center mt-4 md:mt-48 lg:mt-40 md:w-1/3">
          <div className="text-sm text-gray-800 p-2 ml-4 md:ml-0">
            <span className="font-semibold">Duration</span>
            <p className="italic mt-2">{course.duration}</p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default DataScienceAi;
